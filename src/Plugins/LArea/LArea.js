/**
 * LArea 2及联动或者3级联动或者1级联动
 * 最近修改于： 2017-8-8 16:18:46
 */
window.LArea = (function() {
    ...
    MobileArea.prototype = {
        init: function(params) {
            this.params = params;
            this.trigger = document.querySelector(params.trigger);//显示框
            if(params.valueTo){
              this.valueTo=document.querySelector(params.valueTo);
            }
            this.keys = params.keys;
            this.type = params.type||1;//数据源
            switch (this.type) {
                case 1:
                case 2:
                    break;
       ...
       getData: function(callback) {
        ...
        bindEvent: function() {
            ...
            _self.getData(function() {
               _self.trigger.addEventListener('click', popupArea);//每次点击就组装数据下拉框dom//关闭这是移除掉了的
           });
        },
        //重置节点个数
        setGearTooth: function(data) {
            ...
            if (l > 0) {
                ...
                var itemStr = "";
                for (var i = 0; i < l; i++) {
                    itemStr += "<div class='tooth' ids='" + item[i][this.keys['ids']] + "' ref='" + item[i][this.keys['id']] + "'>" + item[i][this.keys['name']] + "</div>";
                }
        ...
        finish: function(e) {
            ...
            var cityIds = area_city.childNodes[cityVal].getAttribute('ids');
            ...
            var countyVal='',countyText='',countyCode='';
        ...
        close: function(e) {
			...
			var gearAreas = document.querySelectorAll('.gearArea');
			for(var i = 0;i<gearAreas.length;i++){
